...
var NpmInstallationManager = (function () {
    ...
    NpmInstallationManager.prototype.addToCache = function (packageName, version) {
        ...
        return (function () {
            ...
            var cachedPackageData;
            if (!_this.$fs.exists(cachedPackagePath).wait() || !_this.$fs.exists(path.join(cachedPackagePath, "framework")).wait()) {
                cachedPackageData = _this.addToCacheCore(packageName, version).wait();
            }
            var realVersion = (cachedPackageData && cachedPackageData.version) || version;
            if (!_this.isShasumOfPackageCorrect(packageName, realVersion).wait()) {
                cachedPackageData = _this.addCleanCopyToCache(packageName, version).wait();
            }
    ...
    NpmInstallationManager.prototype.getLatestCompatibleVersion = function (packageName) {
        var _this = this;
        return (function () {
            var cliVersionRange = "~" + _this.$staticConfig.version;
            ...
            var versions = data && data[latestVersion] && data[latestVersion].versions;
    ...
    NpmInstallationManager.prototype.addCleanCopyToCache = function (packageName, version) {
        ...
        return (function () {
            ...
            var cachedPackageData = _this.addToCacheCore(packageName, version).wait();
            if (!_this.isShasumOfPackageCorrect(packageName, cachedPackageData.version).wait()) {
                _this.$errors.failWithoutHelp("Unable to add package " + packageName + " with version " + cachedPackageData.version + " to npm cache. Try cleaning your cache and execute the command again.");
            }
            return cachedPackageData;
        }).future()();
    };
    NpmInstallationManager.prototype.addToCacheCore = function (packageName, version) {
        var _this = this;
        return (function () {
            var cachedPackageData = _this.$npm.cache(packageName, version).wait();
            var packagePath = path.join(_this.getCacheRootPath(), packageName, cachedPackageData.version, "package");
            if (!_this.isPackageUnpacked(packagePath, packageName).wait()) {
                _this.cacheUnpack(packageName, cachedPackageData.version).wait();
            }
            return cachedPackageData;
        }).future()();
    };
    ...
    NpmInstallationManager.prototype.installCore = function (packageName, pathToSave, version) {
        ...
        return (function () {
            if (_this.$options.frameworkPath) {
                ...
                var cachedData = _this.addToCache(packageName, version).wait();
                var packageVersion = (cachedData && cachedData.version) || version;
                return _this.getCachedPackagePath(packageName, packageVersion);
            }
        }).future()();
    };
