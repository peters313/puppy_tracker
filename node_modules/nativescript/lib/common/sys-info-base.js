...
var SysInfoBase = (function () {
    ...
    SysInfoBase.prototype.getJavaVersion = function () {
        var _this = this;
        return (function () {
            if (!_this.javaVerCache) {
                try {
                    var output = _this.$childProcess.spawnFromEvent("java", ["-version"], "exit").wait().stderr;
                    _this.javaVerCache = /(?:openjdk|java) version \"((?:\d+\.)+(?:\d+))/i.exec(output)[1];
                }
                catch (e) {
                    _this.javaVerCache = null;
                }
            }
            return _this.javaVerCache;
        }).future()();
    };
    SysInfoBase.prototype.getJavaCompilerVersion = function () {
        var _this = this;
        return (function () {
            if (!_this.javaCompilerVerCache) {
                try {
                    ...
                    _this.javaCompilerVerCache = output ? /javac (.*)/i.exec(output.stderr)[1] : null;
                }
                catch (e) {
                    _this.javaCompilerVerCache = null;
                }
            }
            return _this.javaCompilerVerCache;
        }).future()();
    };
    SysInfoBase.prototype.getXCodeVersion = function () {
        var _this = this;
        return (function () {
            if (!_this.xCodeVerCache) {
                try {
                    _this.xCodeVerCache = _this.$hostInfo.isDarwin ? _this.exec("xcodebuild -version") : null;
                }
                catch (e) {
                    _this.xCodeVerCache = null;
                }
            }
            return _this.xCodeVerCache;
        }).future()();
    };
    SysInfoBase.prototype.getNodeGypVersion = function () {
        var _this = this;
        return (function () {
            if (!_this.nodeGypVerCache) {
                try {
                    _this.nodeGypVerCache = _this.exec("node-gyp -v");
                }
                catch (e) {
                    _this.nodeGypVerCache = null;
                }
            }
            return _this.nodeGypVerCache;
        }).future()();
    };
    SysInfoBase.prototype.getXCodeProjGemLocation = function () {
        var _this = this;
        return (function () {
            if (!_this.xcodeprojGemLocationCache) {
                try {
                    _this.xcodeprojGemLocationCache = _this.$hostInfo.isDarwin ? _this.exec("gem which xcodeproj") : null;
                }
                catch (e) {
                    _this.xcodeprojGemLocationCache = null;
                }
            }
            return _this.xcodeprojGemLocationCache;
        }).future()();
    };
    SysInfoBase.prototype.getITunesInstalled = function () {
        var _this = this;
        return (function () {
            if (!_this.itunesInstalledCache) {
                try {
                    _this.itunesInstalledCache = _this.$iTunesValidator.getError().wait() === null;
                }
                catch (e) {
                    _this.itunesInstalledCache = null;
                }
            }
            return _this.itunesInstalledCache;
        }).future()();
    };
    SysInfoBase.prototype.getCocoapodVersion = function () {
        var _this = this;
        return (function () {
            if (!_this.cocoapodVersionCache) {
                try {
                    if (_this.$hostInfo.isDarwin) {
                        var cocoapodVersion = _this.exec("pod --version");
                        if (cocoapodVersion) {
                            ...
                            _this.cocoapodVersionCache = cocoapodVersion;
                        }
                    }
                }
                catch (e) {
                    _this.cocoapodVersionCache = null;
                }
            }
            return _this.cocoapodVersionCache;
        }).future()();
    };
    SysInfoBase.prototype.getSysInfo = function (pathToPackageJson, androidToolsInfo) {
        ...
        return (function () {
            if (!_this.sysInfoCache) {
                ...
                res.nodeGypVer = _this.getNodeGypVersion().wait();
                res.xcodeVer = _this.getXCodeVersion().wait();
                res.xcodeprojGemLocation = _this.getXCodeProjGemLocation().wait();
                res.itunesInstalled = _this.getITunesInstalled().wait();
                res.cocoapodVer = _this.getCocoapodVersion().wait();
