...
var AndroidEmulatorServices = (function () {
    function AndroidEmulatorServices($logger, $emulatorSettingsService, $errors, $childProcess, $fs, $staticConfig, $devicePlatformsConstants, $logcatHelper, $options, $utils, $injector) {
        this.$logger = $logger;
    ...
    AndroidEmulatorServices.prototype.startEmulator = function () {
        var _this = this;
        return (function () {
            ...
            var image = _this.getEmulatorImage().wait();
            if (image) {
                emulatorId = _this.startEmulatorInstance(image).wait();
    ...
    AndroidEmulatorServices.prototype.runApplicationOnEmulator = function (app, emulatorOptions) {
        var _this = this;
        return (function () {
            var emulatorId = _this.startEmulator().wait();
            _this.runApplicationOnEmulatorCore(app, emulatorOptions.appId, emulatorId).wait();
        }).future()();
    };
    ...
    AndroidEmulatorServices.prototype.getDeviceAndroidDebugBridge = function (deviceIdentifier) {
    ...
    AndroidEmulatorServices.prototype.runApplicationOnEmulatorCore = function (app, appId, emulatorId) {
        var _this = this;
        return (function () {
            _this.$logger.info("installing %s through adb", app);
            var adb = _this.getDeviceAndroidDebugBridge(emulatorId);
            var childProcess = adb.executeCommand(["install", "-r", app], { returnChildProcess: true }).wait();
            _this.$fs.futureFromEvent(childProcess, "close").wait();
            _this.unlockScreen(emulatorId).wait();
            _this.$logger.info("running %s through adb", app);
            var androidDebugBridgeCommandOptions = {
    ...
    AndroidEmulatorServices.prototype.unlockScreen = function (emulatorId) {
        var adb = this.getDeviceAndroidDebugBridge(emulatorId);
        var childProcess = adb.executeShellCommand(["input", "keyevent", "82"], { returnChildProcess: true }).wait();
    ...
    AndroidEmulatorServices.prototype.getNameFromSDKEmulatorId = function (emulatorId) {
        ...
        client.on('data', function (data) {
            ...
            var name = _this.getEmulatorNameFromClientOutput(output);
    ...
    AndroidEmulatorServices.prototype.getEmulatorNameFromClientOutput = function (output) {
        var lines = _.map(output.split(os_1.EOL), function (line) { return line.trim(); });
        var name;
        var firstIndexOfOk = _.indexOf(lines, "OK");
        if (firstIndexOfOk < 0) {
            return null;
        }
        var secondIndexOfOk = _.indexOf(lines, "OK", firstIndexOfOk + 1);
    ...
    AndroidEmulatorServices.prototype.getRunningAvdEmulators = function (adbDevicesOutput) {
        return (function () {
            var emulatorDevices = [];
            _.each(adbDevicesOutput, function (device) {
    ...
    AndroidEmulatorServices.prototype.getAllRunningEmulators = function () {
        var _this = this;
        return (function () {
            var outputRaw = _this.$childProcess.execFile(_this.adbFilePath, ['devices']).wait().split(os_1.EOL);
            var emulators = _this.getRunningAvdEmulators(outputRaw).wait().concat(_this.getRunningGenymotionEmulators(outputRaw).wait());
            return emulators;
        }).future()();
    };
    AndroidEmulatorServices.prototype.getRunningEmulators = function () {
        ...
        return (function () {
            var outputRaw = _this.$childProcess.execFile(_this.adbFilePath, ['devices']).wait().split(os_1.EOL);
            if (_this.$options.geny) {
                return _this.getRunningGenymotionEmulators(outputRaw).wait();
            }
            else {
                return _this.getRunningAvdEmulators(outputRaw).wait();
            }
        }).future()();
    };
